...
HARRIS_KSIZE      = 3
HARRIS_K          = 0.04
HARRIS_THRESHOLD  = 0.01   # fraction of max response to keep
HARRIS_MAX_PTS    = 1000
HARRIS_MIN_DIST   = 7
...
def harris_detect(gray):
    ...
    # Suppress near-duplicate points (simple grid suppression)
    selected = []
...
# How often to re-detect features (frames)
...
def process_video(input_path: str, output_path: str):
    ...
    while True:
        ...
        if prev_pts is None or len(prev_pts) < 50 or frame_idx % REDETECT_INTERVAL == 0:
            mask     = np.zeros_like(frame)   # clear trails on redetect
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Visualise LK optical flow on a video")
